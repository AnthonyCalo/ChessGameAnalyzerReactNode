...
var Engine = function(boardSize, lightSquare, darkSquare, selectColor) {
  ...
  */
  ...
  function evaluate() {
    ...
    for (let piece = P; piece <= k; piece++) {
      for (var pieceIndex = 0; pieceIndex < pieceList[piece]; pieceIndex++) {
  ...
  function searchPosition(depth) {
    ...
    for (let currentDepth = 1; currentDepth <= depth; currentDepth++) {
      ...
      if (typeof(document) != 'undefined') {
        ...
        var guiScore = uciScore;
        var guiDepth = info.split('depth ')[1].split(' ')[0];
        var guiPv = info.split('pv ')[1];
        var guiTime = info.split('time ')[1].split(' ')[0];
      }
      ...
      if (validMove) {
        ...
        if (typeof(document) != 'undefined') {
          ...
          if (userTime) {
            ...
            console.log("hopefully not an issue");
          }
  ...
  return {
    ...
    // GUI methods
    drawBoard: function() { try { return this.drawBoard(); } catch(e) { guiError('.drawBoard()'); } },
    updateBoard: function() { try { return this.updateBoard(); } catch(e) { guiError('.updateBoard()'); } },
    movePiece: function(userSource, userTarget, promotedPiece) { try { this.movePiece(userSource, userTarget, promotedPiece); } catch(e) { guiError('.movePiece()'); } },
    flipBoard: function() { try { this.flipBoard(); } catch(e) { guiError('.flipBoard()'); } },
    ...
    pgn: function() { return this.getGamePgn(); },
